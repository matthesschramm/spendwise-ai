...
export const parseCSV = (text: string): Transaction[] => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Check for the exclusion phrases as early as possible on the raw line
    // This handles both quoted and unquoted commas robustly
    const upperLine = rawLine.toUpperCase();
    if (upperLine.includes("PAYMENT RECEIVED, THANK YOU")) continue;
    if (upperLine.includes("TRANSFER TO XX")) continue;
    if (upperLine.includes("TRANSFER TO MATTHES")) continue;
